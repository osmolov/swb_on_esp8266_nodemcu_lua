...
document.getElementsByName(\"delete\")[0].disabled = (getSelectedCount() > 0) ? false : true;\n\
  ...
  script += refreshTempMessage("spiffs");
...
Upload new file:<br/>\n");
  ...
  page += F("<p>");
  page += F("<input id=\"new_prg_name\" name=\"new_prg_name\" type=\"text\" value=\"");
  page += F("new_pgr_");
  page += F("\" maxlength=\"15\" style=\"width: 75px; text-align: center;\" /> ");
  page += F("<input type=\"button\" id=\"butt_create\" value=\"Создать программу\" onclick=\"openUrl('/switch?Prg_create='+new_prg_name.value);location.reload(true);\" >\n");
  page += F("<p>");
...
void h_wifi()//wifi
{
  ...
  message += F("};\n ");
  message += refreshTempMessage("wifi");
  message += F("</script>");
  message += F("</head>\n");
  message += F("<body>\n");
